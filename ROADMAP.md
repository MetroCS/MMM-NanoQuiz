...
**Current increment:** A CLI validator checks a quiz JSON file against the same `QuizValidator` the module uses at runtime, without any MagicMirror or browser dependency. `validateQuizFile(filePath)` reads and parses the file and returns a `ValidationResult`; `runValidateQuizCli(argv, { validate, writeLine, writeErrorLine })` is the testable core (usage message when no path is given, error reporting when reading/parsing/validating fails, one formatted diagnostic line per issue via the shared `formatDiagnostic`, a summary line, and a process exit code of `0` only when the result is valid); `bin/validate-quiz.js` is the thin executable entry point wired to real `process.argv`/`console`/`process.exitCode`, run via `npm run validate -- <path>` or the `nanoquiz-validate` bin. `formatDiagnostic` was extracted out of the adapter's `validateNanoQuizItems` into `src/validation/formatDiagnostic.js` so the CLI and the running module report diagnostics identically. `bin/validate-quiz.js`'s placement follows [ADR-004](docs/architecture/ADR-004-host-entry-points.md): entry points are named and located per their host platform's own convention, not a project-invented scheme.

A terminal preview satisfies this milestone's "standalone preview path" without pulling in browser rendering (which stays deferred to Milestone 8 as "standalone browser rendering," per the existing scope split). `runPreviewQuizCli(argv, { validate, loadEngineOptions, createEngine, writeLine, writeErrorLine })` validates the file the same way `runValidateQuizCli` does, refusing to start if the file is unreadable/unparseable or has zero valid items, then hands the validated items to a real `QuizEngine` — the first host to drive that engine outside the MagicMirror adapter — and renders its autonomous, real-timed snapshots to the terminal via `createPreviewSnapshotFormatter()`. That formatter is deliberately stateful (not a pure per-snapshot function): the engine emits a snapshot on entering the eliminating phase before anything is eliminated, and folds the final elimination into the same snapshot that reveals the answer, so turning that stream into one line per actual change requires tracking which eliminated indexes have already been reported, resetting on each new question. `bin/preview-quiz.js` is the corresponding thin entry point, run via `npm run preview -- <path>` or the `nanoquiz-preview` bin; like the module it previews, it doesn't exit on its own and is meant to be stopped with Ctrl+C.

Previewing with the engine's built-in defaults would silently diverge from a quiz author's actual configured pacing, so `loadPreviewEngineOptions(configSourcePath, { moduleName, defaultConfigPath, readJsonFile, importConfigModule, fileExists, writeErrorLine })` lets the preview pick up real `timing`/`randomizeQuestions`/`randomizeChoices`/`avoidImmediateRepeats` settings instead. An explicit `.json` argument is treated as just the config values themselves (the shape of a module entry's `config: { ... }`, unwrapped); anything else explicit is treated as a real `config.js`, dynamically imported and searched for the `MMM-NanoQuiz` module entry. With no argument, the same `config.js` extraction is attempted against the standard install layout's default location two directories up from the module root, falling back to `{}` (engine defaults) silently if it's missing — but an explicitly given source that fails to load, parse, or match is a real error, the same as an invalid quiz file, since it was asked for by name. Because `QuizEngine`'s constructor already deep-merges partial `timing` over its own defaults and independently defaults the other flags, the extracted config object is handed to it as-is, with no separate defaults duplicated in the CLI layer.
...
**Current increment:** The project adopts Semantic Versioning and ships `1.0.0`. [ADR-005](docs/architecture/ADR-005-versioning-and-compatibility.md) defines, concretely for this project, what counts as a breaking (major), additive (minor), or internal (patch) change across `config.js` options, MagicMirror notifications, documented CSS hooks, the quiz JSON content schema, CLI commands/flags/exit codes, and the framework contracts other adapters depend on (`QuizItem`, `QuizValidator`/`ValidationResult`/`Diagnostic`, `QuizSource`, `QuizEngine`, `PresentationStrategy`). [`CHANGELOG.md`](CHANGELOG.md) records releases in Keep a Changelog format, covering `0.1.0` and this `1.0.0` release. [`CONTRIBUTING.md`](CONTRIBUTING.md) operationalizes both ADR-002's development process and ADR-005's compatibility policy for contributors: how to set up, the expected documentation/tests/verification cycle, branch and PR conventions, and classifying a change as major/minor/patch before opening a PR. The broader public-API/config audit remains open for a future increment of this milestone.
